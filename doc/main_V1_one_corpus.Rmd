...
#option to substitute alphabet letters with all character symbols found in the ground truth: increase of execution time
...
ocr.true.error = ocr.error[not.in.dictionary.boolean] #5576
...
n.topics = 12 #optimal number of clusters based on the result above, might also be 11
...
for (i in 1:length(candidates.list.nonempty[[15]])){
  ...
  prob.word.candidate = get.probability.word(candidates.list.nonempty[[15]][i], beta.matrix)#P(w|t_k)vector of 12 conditional probabilities
...
######
ocr.final.vec.char = vecsets::vintersect(unlist(str_extract_all(ocr.final.vec, "[:graph:]")), unlist(str_extract_all(ground.truth.vec, "[:graph:]")))

ocr.vec.char = vecsets::vintersect(unlist(str_extract_all(ocr.vec, "[:graph:]")), unlist(str_extract_all(ground.truth.vec, "[:graph:]")))

length(ocr.final.vec.char)/length(ground.truth.vec.char)

length(ocr.final.vec.char)/length(ocr.vec.char)
######
...
ocr.performance.table["character_wise_recall","ocr"] = length(ocr.vec.char)/length(ground.truth.vec.char)
ocr.performance.table["character_wise_precision","ocr"] = length(ocr.vec.char)/length(ocr.vec.char)
ocr.performance.table["character_wise_recall","final_ocr"] = length(ocr.final.vec.char)/length(ground.truth.vec.char)
ocr.performance.table["character_wise_precision","final_ocr"] = length(ocr.final.vec.char)/length(ocr.vec.char)
