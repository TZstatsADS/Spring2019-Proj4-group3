...
score_ex = candidate.word.score(ocr.true.error.nonempty[[4]], candidates.list.nonempty[[4]], probability.topic, confusion.prob, beta.matrix)
candidates.list.nonempty[[4]][which.max(score_ex)]
...
ocr.correct = function(ocr.true.error.nonempty, candidates.list.nonempty, probability.topic, confusion.prob, beta.matrix){
  ...
  for (i in 1:length(candidates.list.nonempty)){
    candidate_score = lapply(ocr.true.error.nonempty[[4]], candidate.word.score, candidates.list.nonempty[[i]], probability.topic = probability.topic, confusion.prob = confusion.prob, beta.matrix = beta.matrix)
    #
    #print(candidates.list.nonempty[[i]])
    #print(candidate.word.score)
    candidate.word.score.max = candidates.list.nonempty[[4]][which.max(candidate_score)]
